...
class SentimentDescriptionAugmenter:
    ...
    def generate_descriptions_batch(self, pixel_values_list, num_patches_list, sentiments):
        ...
        # Process each description type with batch inference
        for strategy in self.strategy_list:
            # Prepare batch of questions for this strategy
            questions = [self.PROMPT_TEMPLATES[strategy][sentiment.lower()] for sentiment in sentiments]
            ...
            try:
                ...
                # Fallback: use error message for all images in batch
...
def process_dataset(dataset_name, dataset_root, split, model, tokenizer, augmenter, batch_size=8):
    ...
    for batch_idx in tqdm(range(num_batches), desc=f"{dataset_name}/{split}"):
        ...
        batch_num_patches = []
        batch_sentiments = []
        batch_valid_indices = []  # Track which items in batch are valid
        ...
        for line_idx, line in enumerate(batch_lines):
            ...
            try:
                ...
                if not image_path.exists():
                    ...
                    # Store original data for missing images
...
def main():
    ...
    for dataset_name in args.datasets:
        for split in args.splits:
            process_dataset(dataset_name, args.dataset_root, split, model, tokenizer, augmenter, args.batch_size)
